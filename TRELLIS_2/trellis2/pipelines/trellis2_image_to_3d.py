...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def __init__(
        ...
        dino: str='facebook/dinov3-vitl16-pretrain-lvd1689m',
        dino_moudel: str='facebook/dinov3-vitl16-pretrain-lvd1689m',
        birefnet_repo="ZhengPeng7/BiRefNet",
        birefnet_ckpt: str = None,
    ):
        if models is None:
            return
        ...
        self.birefnet_repo=birefnet_repo
        self.birefnet_ckpt=birefnet_ckpt
    ...
    def from_pretrained(path: str,) -> "Trellis2ImageTo3DPipeline":
        ...
        # new_pipeline._init_rembg_model(args['rembg_model'])
        # new_pipeline._init_image_cond_model(args['image_cond_model'])

        return new_pipeline
    
    def _init_rembg_model(self, name: str):
        """
        Initialize the rembg model.
        """
        if self.birefnet_ckpt is None:
            pass
        else:
            name_= name["args"]["model_name"]
            print(f"Loading rembg model: {name_}")
            self.rembg_model=BiRefNet(self.birefnet_repo,self.birefnet_ckpt)
    ...
    def _init_image_cond_model(self, name: str):
        ...
        if self.birefnet_ckpt is None:
            ...
            print(f"Loading image conditioning model: {name['args']['model_name']}")
            self.image_cond_model = torch.hub.load(self.dino_moudel,name["args"]["model_name"], weights=self.dino,source="local",pretrained=True)
    ...
    def run(
        ...
        **kwargs
        ...
        cond_512=image[512]
        cond_1024=image[1024] if pipeline_type != '512' else None
