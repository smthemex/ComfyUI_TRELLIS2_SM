...
class DinoV3FeatureExtractor:
    ...
    """
    def __init__(self, model_name: str,ckpt=None, image_size=512):
        self.model_name = model_name
        if ckpt is not None:
            config_ = DINOv3ViTConfig.from_pretrained(model_name)
            self.model=DINOv3ViTModel(config_)
            state_dict = torch.load(ckpt,map_location='cpu',weights_only=False) if not ckpt.endswith('.safetensors') else load_file(ckpt,device='cpu')
